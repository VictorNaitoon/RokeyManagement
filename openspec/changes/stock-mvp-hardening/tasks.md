...
- [x] 1.2 Add `ImportCsvResponse` + `MovimientoStockListResponse` wrapper | Spec: CI-01, IA-01 | Files: `backend/API/DTO/Response/Productos/ImportCsvResponse.cs` (create), `MovimientoStockResponse.cs` (wrap) | AC: Given 25 movimientos When GET page1/10 Then 200 shape {items,total,page,pageSize} | Deps: none | Est: 0.5d
- [x] 1.3 Add CsvHelper 31.x to `API.csproj` | Spec: CI-01 | Files: `backend/API/API.csproj` | AC: Given `dotnet restore` When build Then CsvHelper present | Deps: none | Est: 0.25d
...
- [x] 2.6 Align controller roles Dueño+Gerente allow, Empleado/SuperAdmin 403, wrap movimientos, import 200/207 | Spec: SA-02, CI-03, IA-02, PC-01 | Files: `backend/API/Controllers/Productos/ProductoController.cs` | AC: Given Empleado When POST ajuste/import or GET movimientos Then 403; cross-tenant 404; `GET movimientos` returns wrapper not bare List | Deps: 2.2,2.4,2.5 | Est: 1d — PR3 IA slice DONE (GET movimientos Dueño||Gerente 403 Empleado/SuperAdmin, wrapper MovimientoStockListResponse, tenant 404, page/pageSize validate max100, ProducesResponseType 200/400/403/404); PR4 import 200/207 DONE (POST import/csv Dueño||Gerente 403 Empleado/SuperAdmin, 5MB→413/400, 207 partial, ProducesResponseType ImportCsvResponse 200/207/400/413)
...
- [x] 3.2 Add `ajusteStockSchema` + Ajuste/Import/Movimientos pagination types | Spec: SA-01, IA-01, CI-01 | Files: `apps/admin-panel/src/lib/schemas/producto.schema.ts`, `apps/admin-panel/src/types/producto.types.ts` | AC: Given invalid motivo<5 When validate Then zod error | Deps: 1.1 | Est: 0.25d
...
- [x] 3.4 Wire `ProductosPage.tsx` dialogs, strip StockActual from PUT, gate Historial/Import by role | Spec: PC-02, IA-02, CI-03 | Files: `apps/admin-panel/src/pages/productos/ProductosPage.tsx` | AC: Given Empleado views page Then Historial/Import hidden; PUT never sends StockActual | Deps: 3.3 | Est: 0.5d

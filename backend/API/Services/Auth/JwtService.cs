...
namespace API.Services.Auth
{
    public interface IJwtService
    {
        ...
        /// <summary>
        /// Genera un access token a partir de claims existentes (usado en refresh token)
        /// </summary>
        string GenerateTokenFromRefreshClaims(int userId, string email, int negocioId, string rol);
    }
    ...
    public class JwtService : IJwtService
    {
        ...
        /// <summary>
        /// Genera un access token a partir de claims existentes.
        /// Usado durante refresh token para mantener la sesión sin re-autenticación.
        /// </summary>
        public string GenerateTokenFromRefreshClaims(int userId, string email, int negocioId, string rol)
        {
            var key = new SymmetricSecurityKey(
                Encoding.UTF8.GetBytes(_configuration["Jwt:Key"] ?? "RoKeySuperSecretKey2026!@#$%^&*()")
            );

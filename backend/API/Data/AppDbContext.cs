...
namespace API.Data
{
    public class AppDbContext : DbContext
    {
        ...
        // --- CARRITO INTERNO ---
        public DbSet<CarritoInterno> CarritosInternos { get; set; }
        public DbSet<CarritoInternoItem> CarritosInternosItems { get; set; }
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<MovimientoStock>()
                ...
                .OnDelete(DeleteBehavior.Restrict); // No borrar el movimiento de stock si el negocio es borrado, para mantener el historial.
            ...
            // --- AUDITORÍA: IdUsuarioModificador ---
            // Venta -> UsuarioModificador
            modelBuilder.Entity<Venta>()
                .HasOne(v => v.UsuarioModificador)
                .WithMany()
                .HasForeignKey(v => v.IdUsuarioModificador)
                .OnDelete(DeleteBehavior.Restrict);

            // Compra -> UsuarioModificador
            modelBuilder.Entity<Compra>()
                .HasOne(c => c.UsuarioModificador)
                .WithMany()
                .HasForeignKey(c => c.IdUsuarioModificador)
                .OnDelete(DeleteBehavior.Restrict);

            // Producto -> UsuarioModificador
            modelBuilder.Entity<Producto>()
                .HasOne(p => p.UsuarioModificador)
                .WithMany()
                .HasForeignKey(p => p.IdUsuarioModificador)
                .OnDelete(DeleteBehavior.Restrict);

            // Presupuesto -> UsuarioModificador
            modelBuilder.Entity<Presupuesto>()
                .HasOne(p => p.UsuarioModificador)
                .WithMany()
                .HasForeignKey(p => p.IdUsuarioModificador)
            ...
            // Usuario -> UsuarioCreador (self-referencing)
            ...
            // Usuario -> UsuarioModificador (self-referencing)
            modelBuilder.Entity<Usuario>()
                .HasOne(u => u.UsuarioModificador)
                .WithMany()
                .HasForeignKey(u => u.IdUsuarioModificador)
                .OnDelete(DeleteBehavior.Restrict);

            // --- CARRITO INTERNO ---
            // CarritoInterno -> Negocio
            modelBuilder.Entity<CarritoInterno>()
                .HasOne(c => c.Negocio)
                .WithMany()
                .HasForeignKey(c => c.IdNegocio)
                .OnDelete(DeleteBehavior.Restrict);

            // CarritoInterno -> Usuario (vendedor)
            modelBuilder.Entity<CarritoInterno>()
                .HasOne(c => c.Usuario)
                .WithMany()
                .HasForeignKey(c => c.IdUsuario)
            ...
            // CarritoInternoItem -> CarritoInterno (cascade delete)
            modelBuilder.Entity<CarritoInternoItem>()
                .HasOne(i => i.CarritoInterno)
                .WithMany(c => c.Items)
                .HasForeignKey(i => i.CarritoInternoId)
                .OnDelete(DeleteBehavior.Cascade);

            // CarritoInternoItem -> Producto
            modelBuilder.Entity<CarritoInternoItem>()
                .HasOne(i => i.Producto)
                .WithMany()
            ...
            // Índices para CarritoInterno
            modelBuilder.Entity<CarritoInterno>()
                .HasIndex(c => c.IdNegocio);
            ...
            // Índice para CarritoInternoItem
            modelBuilder.Entity<CarritoInternoItem>()
                .HasIndex(i => i.CarritoInternoId);

            // --- REFRESH TOKEN ---
            // RefreshToken -> Usuario (FK sin navegación)
            ...
            // Índice en Token para búsquedas únicas
            modelBuilder.Entity<RefreshToken>()
                .HasIndex(r => r.Token)
                .IsUnique();

            // Índice compuesto para búsquedas de tokens activos por usuario
            modelBuilder.Entity<RefreshToken>()
